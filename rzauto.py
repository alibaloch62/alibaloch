...
try:
    ...
    if "Version-1.5" in response:
        ...
        if kex in response:
            ...
            linex()
            print(" Your key: "+fkeyx);subprocess.check_output(["am", "start", "https://api.whatsapp.com/send?phone=+923267069530&text="+(" Hi AKING sir i want Buy Auto Create Ids Tool Please Approve My Token\n Token:- "+ fkeyx)]);time.sleep(2)
            exit('\n Run:  python rzauto.py')
            sys.exit()
    else:
